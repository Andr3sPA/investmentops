...
(TASKS.md, Fase 6, "Reportes"). No prueba de nuevo las demás secciones
(ya cubiertas en `test_reports_markdown.py`/
`test_reports_markdown_trend.py`/`test_reports_markdown_news.py`/
`test_reports_markdown_comparables.py`), ni los agentes de estrategia en
sí (`investmentops.analysis_engines.value`/`growth`/`quality`, ya
cubiertos en sus propios archivos de prueba) ni su registro en el
orquestador (`run_value_engine`/`run_growth_engine`/`run_quality_engine`,
ya cubiertos en `test_core_orchestrator_strategies.py`/
`test_core_orchestrator_strategies_integration.py`).
"""
...
def test_render_includes_section_header() -> None:
    ...
    assert "## Lecturas por estrategia de inversión" in output
...
def test_render_shows_section_after_comparables() -> None:
    ...
    assert output.index("## Comparables del sector") < output.index(
...
def test_render_includes_empty_subheaders_for_all_three_strategies() -> None:
    ...
    assert "### Value investing" in output
    assert "### Growth investing" in output
    assert "### Calidad (quality investing)" in output
...
def test_render_shows_strategy_subheaders_in_order() -> None:
    ...
    assert output.index("### Value investing") < output.index("### Growth investing")
    assert output.index("### Growth investing") < output.index(
...
def test_render_is_the_last_section_of_the_document() -> None:
    ...
    assert output.rstrip("\n").endswith(
        output[output.index("## Lecturas por estrategia de inversión"):].rstrip("\n")
    )
...
def test_render_keeps_empty_strategy_subsections_when_agents_absent() -> None:
    ...
    section_start = output.index("### Value investing")
    section_body = output[section_start:].replace("### Value investing", "")
    section_body = section_body.replace("### Growth investing", "")
    section_body = section_body.replace("### Calidad (quality investing)", "")
...
def test_render_includes_value_findings_under_its_own_subsection() -> None:
    ...
    section_start = output.index("### Value investing")
    section_end = output.index("### Growth investing")
...
def test_render_includes_growth_findings_under_its_own_subsection() -> None:
    ...
    section_start = output.index("### Growth investing")
    section_end = output.index("### Calidad (quality investing)")
...
def test_render_includes_quality_findings_under_its_own_subsection() -> None:
    ...
    section_start = output.index("### Calidad (quality investing)")
...
def test_render_shows_all_three_strategies_when_all_present() -> None:
    ...
    value_start = output.index("### Value investing")
    growth_start = output.index("### Growth investing")
    quality_start = output.index("### Calidad (quality investing)")
...
def test_render_does_not_merge_strategies_into_a_single_reading() -> None:
    """Cada estrategia debe quedar en su propia subsección, nunca fusionada
    con otra ni con un veredicto único (ver GOALS.md)."""
    result = assemble_research_result(
        "AAPL",
        [
...
def test_render_strategy_sections_ignore_other_analysis_results() -> None:
    ...
    section_start = output.index("## Lecturas por estrategia de inversión")
...
def test_render_includes_growth_supporting_metrics() -> None:
    ...
    assert "- revenue_trend: creciente" in output
...
def test_render_includes_real_ai_provenance_for_each_strategy() -> None:
    ...
    assert "**Generado por:** anthropic (claude-haiku-4-5) el" in output
