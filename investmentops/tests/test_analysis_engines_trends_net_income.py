"""Pruebas para el cálculo de variación de beneficios periodo a periodo
(investmentops.analysis_engines.trends.calculate_net_income_growth).

Cubre la tarea "Implementar el cálculo de variación periodo a periodo de
beneficios" (TASKS.md, Fase 3, "Motor de análisis: evolución de ingresos
y beneficios"), sobre la definición ya fijada en
`investmentops/analysis_engines/TREND_METRICS.md`. Mismo patrón de
pruebas ya usado en `test_analysis_engines_trends.py` para
`calculate_revenue_growth`, aplicado a `net_income`. No prueba de nuevo
`calculate_revenue_growth` (ya cubierto en ese archivo), la detección de
tendencia agregada, ni el ensamblado del resultado del motor: son tareas
...
def _statement(
    period_end: date, net_income: float, revenue: float = 1_000_000.0
) -> FinancialStatement:
...
def test_calculates_growth_for_two_consecutive_periods() -> None:
    series = FinancialStatementSeries(
        ticker="AAPL",
        statements=[
            _statement(date(2025, 12, 31), 165_000.0),
            _statement(date(2024, 12, 31), 150_000.0),
        ],
    )

    result = calculate_net_income_growth(series)

    assert isinstance(result, NetIncomeGrowthResult)
    assert len(result.points) == 1
    point = result.points[0]
    assert isinstance(point, NetIncomeGrowthPoint)
    assert point.period_end == date(2025, 12, 31)
    assert point.previous_period_end == date(2024, 12, 31)
    assert point.net_income_growth == pytest.approx(0.1)
...
def test_produces_one_point_per_consecutive_pair_across_several_periods() -> None:
    series = FinancialStatementSeries(
        ticker="AAPL",
        statements=[
            _statement(date(2025, 12, 31), 240_000.0),
            _statement(date(2024, 12, 31), 200_000.0),
            _statement(date(2023, 12, 31), 180_000.0),
            _statement(date(2022, 12, 31), 190_000.0),
        ],
    )
...
def test_classifies_growth_as_creciente_when_positive() -> None:
    series = FinancialStatementSeries(
        ticker="AAPL",
        statements=[
            _statement(date(2025, 12, 31), 110_000.0),
...
def test_classifies_growth_as_decreciente_when_negative() -> None:
    series = FinancialStatementSeries(
        ticker="AAPL",
        statements=[
            _statement(date(2025, 12, 31), 90_000.0),
            _statement(date(2024, 12, 31), 100_000.0),
        ],
    )
    ...
    assert result.points[0].net_income_growth == pytest.approx(-0.1)
...
def test_classifies_growth_as_estable_when_exactly_zero() -> None:
    ...
    assert result.points[0].net_income_growth == 0.0
...
def test_uses_abs_of_base_period_so_recovering_losses_reads_as_positive() -> None:
    ...
    assert result.points[0].net_income_growth == pytest.approx(0.5)
    assert result.points[0].classification == "creciente"


def test_base_period_net_income_zero_returns_none_with_warning_for_that_point() -> None:
    series = FinancialStatementSeries(
        ticker="AAPL",
        statements=[
    ...
    point = result.points[0]
    assert point.net_income_growth is None
...
def test_does_not_raise_zero_division_error() -> None:
    ...
    try:
        calculate_net_income_growth(series)
...
def test_only_the_affected_pair_is_none_others_still_calculated() -> None:
    ...
    assert result.points[0].net_income_growth == pytest.approx(1.0)
    assert result.points[0].warning is None
    assert result.points[1].net_income_growth is None
    assert result.points[1].warning is not None


def test_single_period_series_returns_no_points_with_series_level_warning() -> None:
    series = FinancialStatementSeries(
        ticker="AAPL", statements=[_statement(date(2025, 12, 31), 100_000.0)]
    )

    result = calculate_net_income_growth(series)

    assert result.points == ()
    assert result.warnings == (NET_INCOME_SINGLE_PERIOD_WARNING,)
...
def test_net_income_growth_point_is_immutable() -> None:
    series = FinancialStatementSeries(
        ticker="AAPL",
        statements=[
            _statement(date(2025, 12, 31), 110_000.0),
    ...
    with pytest.raises(AttributeError):
        result.points[0].net_income_growth = 99.0  # type: ignore[misc]


def test_net_income_growth_result_is_immutable() -> None:
    series = FinancialStatementSeries(
        ticker="AAPL", statements=[_statement(date(2025, 12, 31), 100_000.0)]
    )
