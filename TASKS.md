...
- [x] Implementar la invocación al proveedor de IA configurado con esas métricas + el prompt. — `invoke_financial_health_agent` en `investmentops/analysis_engines/financial_health.py` (ver PROGRESS.md). Combina tres piezas nuevas de infraestructura reutilizable: `investmentops.analysis_engines.prompts.load_prompt` (carga `prompts/<agent_id>.md` desde disco), `investmentops.ai_providers.factory.build_ai_provider` (traduce el nombre de proveedor resuelto por `resolve_agent_provider` a la instancia concreta de `AIProvider`) y la propia `resolve_agent_provider` ya existente. Invoca `AIProvider.complete(prompt, data=...)` enviando el `FinancialStatement` y las `FinancialHealthMetrics` ya calculadas como `data`, sin que la IA calcule ni corrija ninguna métrica. Devuelve el `AIProviderResponse` crudo; no lo parsea a `AnalysisResult` (tarea siguiente).
